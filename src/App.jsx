...
import PrivacyPage from './components/PrivacyPage'; // 개인정보 처리방침
import TermsPage from './components/TermsPage'; // 이용약관
...
function parseRoute(pathname) {
  ...
  if (pathname.startsWith('/privacy')) return { page: 'privacy' };
  if (pathname.startsWith('/terms')) return { page: 'terms' };
  if (pathname.startsWith('/coincharge')) return { page: 'coincharge' };
  if (pathname.startsWith('/vote')) return { page: 'vote' };
  return {};
}
...
function App() {
  ...
  const [communityNonce, setCommunityNonce] = useState(0); // 커뮤니티 메뉴 재클릭 시 리스트로 리셋하는 신호
  ...
  // 법적 고지 페이지(개인정보/이용약관) URL 동기화 — 공개 URL 직접 접근/공유 지원
  useEffect(() => {
    const p = window.location.pathname;
    if (currentPage === 'privacy' && p !== '/privacy') window.history.replaceState(null, '', '/privacy');
    else if (currentPage === 'terms' && p !== '/terms') window.history.replaceState(null, '', '/terms');
    else if (currentPage !== 'privacy' && currentPage !== 'terms' && (p === '/privacy' || p === '/terms')) window.history.replaceState(null, '', '/');
  }, [currentPage]);
  ...
  const handleNavigate = (page, sectionId = null, categoryFilter = null) => {
    ...
    // 커뮤니티 메뉴 클릭 시 상세→리스트로 리셋하도록 신호 증가
    if (page === 'community') setCommunityNonce((n) => n + 1);
    if (page === 'services') {
      setCurrentPage('home');
  ...
  if (pendingNotice) {
    ...
    switch (currentPage) {
      ...
      case 'admin-hub':
        ...
        break;
      case 'admin':
        content = <AdminDashboardPage isAdmin={isAdmin} currentUserEmail={session?.user?.email} onBack={() => handleNavigate('admin-hub')} onViewProfile={openUserProfile} />;
        break;
      case 'b2brequests':
